...
export default {
  ...
  created() {
    this.fetchFittoCorvini();
  },
  methods: {
    async fetchFittoCorvini() {
      try {
        const response = await axios.get('http://localhost:3001/api/fitto_corvini');
        this.fittoCorvini = response.data;
      } catch (error) {
        console.error('Error fetching fitto corvini:', error);
      }
    },
